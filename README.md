...
- `config/config.json`
  ...
  - `environments.*.defaultParams.limit`: limit per request
    - `api`: API request settings
      - `endpoint`: route appended to the base URL
      - `timeoutMs`: request timeout (10s default)
      - `delayBetweenCallsMs`: enforced pause between calls (500 ms default)
      - `expectedStatusCodes`: accepted HTTP status codes (e.g., 200, 201)
      - `pagination`: pagination behavior controls
        - `mode`: `firstPage` to sample the initial page only, `allPages` to iterate until pagination metadata signals completion
        - `pageParam`: query parameter sent for the current page (default `page`)
        - `pageSizeParam` / `pageSize`: override the page-size parameter/value when paging
        - `maxPages`: safety cap on pages fetched when `mode = allPages` (0 = unlimited)
        - `pauseBetweenPagesMs`: delay between successive page requests to avoid throttling
        - `responseFields.*`: dot-paths to pagination metadata in the response payload (`currentPage`, `totalPages`, `hasNextPage`, `totalCount`)
...
- When pagination mode is `allPages`, aggregates items across every retrieved page before computing pass-rate and diagnostics
